...
function isUsernameExists($emp_number, $table, $conn)
{
    $sql = "SELECT * FROM `$table` WHERE emp_number='$emp_number'; ";
...
function esignup($emp_number, $first_name, $last_name, $job_title, $password)
{
    ...
    if (isUsernameExists($emp_number, 'Employee', $conn)) {
        $error_msg = "Employee is already exists.";
        return false;
    }
...
global $success_msg;
    ...
    if ($result && mysqli_affected_rows($conn) > 0) {
        return true;
          $success_msg="Request Declined Successfully";
    }
