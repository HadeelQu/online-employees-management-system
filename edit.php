...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  for ($i = 0; $i < $total_count; $i++) {
    ...
    if ($tmpFilePath != "") {
      if (!file_exists("./upload_files/" . $request_id)) {
        mkdir("./upload_files/" . $request_id, 0777, true);
      }
     
      $newFilePath = "./upload_files/" . $request_id . "/" . $_FILES['myfile']['name'][$i];
  ...
  if ($uploaded) {
    $result = editRequest($request_id, $service_type, $description, $files);

    if ($result) {
      header("location:request-information-page.php?id=$request_id&emsg=success");
      exit();
    }
  }
}
...
<div class="container2">
  ...
  <form name="FormE" id="editfForm" enctype="multipart/form-data" method="post">
    <fieldset>
      <legend id="edit">
        Edit request
      </legend>
      ...
      <?php if ($request) { ?>
      <div class="container">
      <input type="hidden" id="request_id" name="request_id" value="<?php echo $request['id']; ?>">
        ...
        <select id="service_type" name="service_type" style="width: 100%; height: 30px; margin: 0" required>
          <?php foreach ($services as $service) { ?>
            <option value="<?php echo $service['id']; ?>" <?php ($service['id'] == $request['service_id']) ? "selected" : "" ;?>><?php echo $service['type']; ?></option>
          <?php } ?>
        </select>

        <label>Description : </label>

        <textarea name="description" placeholder="Write a description for your request" required><?php echo $request['description']; ?></textarea>
        <style>
          textarea {
            width: 100%;
            height: 150px;
        ...
        <input id="Button" type="button" value="Request" onclick="vailda();return false;">
